    ...
    <form class="form" style="display:none;" action="{{ route('website.widget.container.update') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="p-2 card-body">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" placeholder="{{ translate('title')}}" name="title" value="{{$widget->title}}" onchange="submit_update(this)"/>
            </div>
            <div class="form-group ">
                <label for="address">{{ translate('Address') }}</label>
                <input type="text" name="address" placeholder="{{ translate('address')}}" class="form-control " value="{{$contact_info->address}}" onchange="submit_update(this)">
            </div>
            <div class="form-group ">
                <label for="phone">{{ translate('Phone') }}</label>
                <input type="text" name="phone" placeholder="{{ translate('phone')}}" class="form-control " value="{{$contact_info->phone}}" onchange="submit_update(this)">
            </div>
            <div class="form-group ">
                <label for="email">{{ translate('Email') }}</label>
                <input type="text" name="email" placeholder="{{ translate('email')}}" class="form-control " value="{{$contact_info->email}}" onchange="submit_update(this)">
            </div>
        </div>
