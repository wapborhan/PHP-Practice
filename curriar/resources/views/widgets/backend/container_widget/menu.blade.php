    ...
    <form class="form" style="display:none;" action="{{ route('website.widget.container.update') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="card-body p-2">
            ...
            <div class="form-group">
                ...
                <select class="form-control" name="menu" onchange="submit_update(this)">
                    <option selected>{{ translate('Choose Menu') }}...</option>
                    @foreach ($menus as $menu_item)
                        <option data-tokens="{{$menu_item->name}}" value="{{$menu_item->id}}" @if($menu_id == $menu_item->id) selected @endif>{{$menu_item->name}}</option>
                    @endforeach
                </select>
            </div>
        </div>
