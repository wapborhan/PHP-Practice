        ...
        <div class="card-body">
            ...
            <div class="form-group">
                ...
                @if($about->logo)
                    ...
                    <a href="{{static_asset($about->logo)}}" target="_blank" class="image-link">
                        <img src="{{static_asset($about->logo)}}" alt="{{ $container_widget->title }}" class="image-preview" style="max-height: 50px;"/>
                    </a>
                @endif
            </div>
            <div class="form-group">
                <label>{{ translate('Email') }}</label>
                <input type="text" class="form-control" placeholder="company email" name="email" value="{{isset($about->email) ? $about->email : ''}}" onchange="submit_update(this)"/>
            </div>
            <div class="form-group">
                <label>{{ translate('Phone') }}</label>
                <input type="text" class="form-control" placeholder="company phone" name="phone" value="{{isset($about->phone) ? $about->phone : ''}}" onchange="submit_update(this)"/>
            </div>
        </div>
