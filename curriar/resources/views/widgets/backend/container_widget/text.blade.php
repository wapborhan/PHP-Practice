    ...
    <form class="form" style="display:none;" action="{{ route('website.widget.container.update') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="card-body p-2">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" placeholder="{{ translate('title')}}" name="title" value="{{$text->title}}" onchange="submit_update(this)"/>
            </div>
            <div class="form-group ">
                <label for="description">{{ translate('Description') }}</label>
                <input type="text" name="description" placeholder="{{ translate('description')}}" class="form-control " value="{{$text->description}}" onchange="submit_update(this)">
            </div>
        </div>
